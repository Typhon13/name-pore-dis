...
void addPlatform(int x, int y, int width, int height) {
    ...
    iInitSprite(&platforms[platformCount].sprite, -1);
...
void loadResources(){
    ...
    iInitSprite(&portal_open.sprite, -1);
    iChangeSpriteFrames(&portal_open.sprite, portal_open.frame, 8);
    iSetSpritePosition(&portal_open.sprite, 1050, 830);
    iScaleSprite(&portal_open.sprite, 1);  

    iInitSprite(&portal.sprite, -1);
    iChangeSpriteFrames(&portal.sprite, portal.frame, 8);
    iSetSpritePosition(&portal.sprite, 1050, 830);
    iScaleSprite(&portal.sprite, 1);


    iInitSprite(&key_yellow.sprite, -1);
    iChangeSpriteFrames(&key_yellow.sprite, key_yellow.frame, 12);
    iSetSpritePosition(&key_yellow.sprite, 500, 400);
    iScaleSprite(&key_yellow.sprite, 0.4);
    
    iInitSprite(&key_red.sprite, -1);
    iChangeSpriteFrames(&key_red.sprite, key_red.frame, 12);
    iSetSpritePosition(&key_red.sprite, 650, 550);
    iScaleSprite(&key_red.sprite, 0.4);

    iInitSprite(&key_blue.sprite, -1);
    iChangeSpriteFrames(&key_blue.sprite, key_blue.frame, 12);
    iSetSpritePosition(&key_blue.sprite, 850, 700);
    iScaleSprite(&key_blue.sprite, 0.4);

    iInitSprite(&angel.sprite, -1);
    iChangeSpriteFrames(&angel.sprite, angel.idle, 24);
    iSetSpritePosition(&angel.sprite, 40, 120);
    iScaleSprite(&angel.sprite, 0.1);
    ...
    iResizeImage(&menuBackground, 1800, 1000);
    ...
    addPlatform(450, 350, 200, 30); 
    addPlatform(600, 500, 200, 30);
    addPlatform(800, 650, 200, 30);
    addPlatform(1000, 800, 200, 30);  
}
...
void initiateJump() {
    if (!angel.inAir) { // Only jump if on ground/platform
        ...
        iChangeSpriteFrames(&angel.sprite, angel.jump, 2); // Assume 25 frames for jump
    }
}
...
void iAnim()
{
    ...
    if (!angel.inAir) { // Only do this if not explicitly jumping
        if (!isOnGroundOrPlatform()) { // If they somehow moved off a platform
            angel.inAir = true; // Set them to inAir so gravity applies
...
{    iClear();
    ...
    iShowLoadedImage(500, 700, &moon);
    iSetColor (0,255,0);
    iTextBold(200, 360,"Press RIGHT arrow to go right");
    iTextBold(200, 345,"Press LEFT arrow to go left");
    iTextBold(200, 330,"Press UP arrow to jump");
    iTextBold(200, 500,"Collect keys to open portal");
    iTextBold(200, 550,"Get into portal to win");
    if (portalcond == 1){
        iTextBold(500, 900,"You Win");
    }
    ...
    iTextBold(200, 800, keyCountBuffer); // Display the string from the buffer
    ...
    iShowLoadedImage(
        ...
        iShowLoadedImage(750, 500, &startButton);
        iShowLoadedImage(750, 350, &optionsButton);
        iShowLoadedImage(750, 200, &exitButton);
        
        // Highlight selected item
        iSetColor(0, 0, 0);
        switch(menuSelection) {
            case 0: iRectangle(745, 495, 310, 110); break; // Start
            case 1: iRectangle(745, 345, 310, 110); break; // Options
            case 2: iRectangle(745, 195, 310, 110); break; // Exit
        }
        
        // Menu instructions
        iSetColor(0, 255, 0);
        iText(800, 100, "Use UP/DOWN arrows to navigate, END to select");
    }
}
...
void iSpecialKeyboard(unsigned char key)
{
    ...
    {  
        ...
        {
            angel.sprite.x -=5;
        if(isKeyPressed(GLUT_KEY_RIGHT))    
        angel.sprite.x -=5;
        if (angel.state != WALK)
            {
                angel.state = WALK;
...
void iSpecialKeyboardUp(unsigned char key)
{
    ...
    // Gravity should continue pulling the player down.
}
int main(int argc, char *argv[])
{
    ...
    iInitialize(1800, 1000, "mankindangel");
    return 0;
}                                                       